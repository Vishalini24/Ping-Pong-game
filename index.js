...
// localStorage.getItem retrieves the value associated with a given key from the browser's local storage.
// If the key is not found in the local storage (i.e., it has never been set before), localStorage.getItem returns null.
// The !== operator to compare null with a string, such as "false" in this case, the comparison will always evaluate 
// to true because null is not equal to any string, including "false".
...
// Alert message when game starts
...
function set_interval() {
  ...
  // Determine which rod lost and assign the ball to it for the next match
  if (rod1_Score < rod2_Score) {
    ball.classList.remove(ball.classList[0]);
    ball.classList.add("animate-top-right");
  } else if (rod2_Score < rod1_Score) {
    ball.classList.remove(ball.classList[0]);
    ball.classList.add("animate-bottom-right");
  } else {
    // If it's a tie or the first match, you can choose any rod to start with
    if (rod1_Score === 0 && rod2_Score === 0) {
      // Randomly assign the ball to one of the rods
      if (Math.random() < 0.5) {
        ball.classList.remove(ball.classList[0]);
        ball.classList.add("animate-bottom-right");
      } else {
      ...
      // In case of a tie, continue with the rod that had the ball last
      if (ball.classList.contains("animate-bottom-right")) {
        ball.classList.remove(ball.classList[0]);
        ball.classList.add("animate-bottom-right");
      } else {
        ball.classList.remove(ball.classList[0]);
